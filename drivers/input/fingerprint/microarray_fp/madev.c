        ...
        case MA_IOC_GVER:
            {            
                ...
                if (copy_to_user((void *)arg, &ma_drv_version, sizeof(unsigned int))) {
                    printk("dantangwei1copy_to_user(..) failed.\n");
                    ret = (-EFAULT);
                }            
                mutex_unlock(&ioctl_lock);
            }
...
int mas_plat_probe(struct platform_device *pdev) {
    ...
    if(ret){
        printk("dantangwei1mas_plat_probe do mas_finger_get_gpio_info");
    }
    ret = mas_finger_set_gpio_info(1);
    if(ret){
        printk("dantangwei1mas_plat_probe do mas_finger_set_gpio_info");
    }
    printk("dantangweiend");
    return ret;
}
...
int mas_probe(struct spi_device *spi) {
    ...
    ret = init_connect();
    if (ret == 0) {
        printk("dantangwei1chip is not connected\n");
        return -1;
    }
#else
	ret = 0;
...
{
    int ret = 0;
	printk("dantangwei1mas_init\n");
    MALOGF("start");
    compatible = 1;
    ret = mas_get_platform();
    if(ret){
	   printk("dantangwei1mas_get_platform");
    }
    return ret;
}
